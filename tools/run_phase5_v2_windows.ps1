...
$RunRoot = Join-Path $Downloads "karzoun-x-phase5-v2"
...
Write-Step "Locating a working Phase 5 Python environment"
$OldWorkspace = Get-ChildItem -Path $Downloads -Directory |
    Where-Object { $_.Name -like "karzoun-x-phase5-*" -and $_.Name -ne "karzoun-x-phase5-v2" } |
    Sort-Object LastWriteTime -Descending |
    Where-Object { Test-Path (Join-Path $_.FullName ".venv\Scripts\python.exe") } |
    Select-Object -First 1

if ($null -eq $OldWorkspace) {
    throw "No existing KARZOUN-X Phase 5 virtual environment was found in Downloads."
}

$PythonExe = Join-Path $OldWorkspace.FullName ".venv\Scripts\python.exe"
Write-Host "Reusing Python environment: $PythonExe" -ForegroundColor Green
...
$V2Runner = Join-Path $RunRoot "scripts\run_phase5_local_llm_v2.py"
if (-not (Test-Path $V2Runner)) {
    throw "Phase 5 v2 runner is missing from current main."
}

$env:PYTHONPATH = Join-Path $RunRoot "src"

Write-Step "Running quality checks on current main"
...
$ResultDir = Join-Path $RunRoot "results\phase5_v2"
New-Item -ItemType Directory -Force -Path $ResultDir | Out-Null
$EnvironmentLines = @(
    "KARZOUN-X Phase 5 v2 local execution environment",
...
Write-Step "Running Phase 5 v2 with visible per-case progress"
$RunLog = Join-Path $RunRoot "results-phase5-v2.log"
& $PythonExe $V2Runner --base-url $OllamaBaseUrl --model $Model 2>&1 |
    Tee-Object -FilePath $RunLog
if ($LASTEXITCODE -ne 0) {
    throw "Phase 5 v2 failed. The log is at $RunLog"
}

Write-Step "Committing machine-generated v2 results"
& git config user.name "Mahmoud Karzoun"
& git config user.email "135722882+mkarson1997@users.noreply.github.com"
& git add results/phase5_v2 results-phase5-v2.log
$Changes = (& git status --porcelain) -join "`n"
if ([string]::IsNullOrWhiteSpace($Changes)) {
    throw "Phase 5 v2 produced no result changes."
}
& git commit -m "results: record Phase 5 v2 local LLM benchmark"
...
if ($LASTEXITCODE -ne 0) {
    $Branch = "phase5-v2-results-" + (Get-Date -Format "yyyyMMdd-HHmmss")
...
Write-Host "KARZOUN-X PHASE 5 V2 COMPLETE" -ForegroundColor Green
Write-Host "Workspace: $RunRoot"
Write-Host "Summary:   $RunRoot\results\phase5_v2\summary.md"
